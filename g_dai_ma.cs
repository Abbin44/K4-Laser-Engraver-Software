...
namespace diao
{
  internal class g_dai_ma
  {
    ...
    private static double[] qu_dian_double(string str)
    {
      ...
      double num2 = Convert.ToDouble(str.Substring(startIndex1 + 1, num1 - startIndex1 - 1));
      numArray[0] = num2;
      int startIndex2 = str.IndexOf("Y", 0, StringComparison.Ordinal);
      int num3 = str.IndexOf(" ", startIndex2, StringComparison.Ordinal);
      if (num3 <= 0)
        num3 = str.Length;
      string str1 = str.Substring(startIndex2 + 1, num3 - startIndex2 - 1);
      int num4 = str1.IndexOf("F", 0, StringComparison.Ordinal);
      if (num4 >= 1)
        str1 = str.Substring(startIndex2 + 1, num4 - 1);
      ...
      foreach (string str in strArray)
      {
        ...
        else if (str[0] == 'M')
        {
          if (str[str.Length - 1] == '3')
            g_dai_ma.zhixian = g_dai_ma.jiaru(g_dai_ma.zhixian, new Point(50001, 0));
          if (str[str.Length - 1] == '5')
            g_dai_ma.zhixian = g_dai_ma.jiaru(g_dai_ma.zhixian, new Point(50000, 0));
        }
      }
